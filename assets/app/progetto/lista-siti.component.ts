...
@Component({
    ...
    styleUrls: ['./lista-siti.component.css']
})
export class ListaSitiComponent implements  OnInit {
    ...
    public siti: Sito[] = [];
    public canali: YouTube[] = [];
    editorPlaceholder = "Scrivi URL del canale YouTube qui...";
    ...
    ngOnInit(){
        ...
        this.progettoService.getElencoYouTube()
        
        .subscribe(
            (siti: YouTube[]) => {
                this.canali = siti;
                console.log('Elenco Canali YouTube crawler: ',  this.canali);
            }
        );
    ...
    OneditorModelChangeUrl(event) { 
     ...
     onSubmit() {
       ...
       // this.progetto = this.progettoService.progetto;
        console.log('Submit: sito >>', youtube);

        this.progettoService.addYouTube(youtube)
            .subscribe(
                data => {
                    console.log(data);
                    alert(`Sito aggiunto: ${JSON.stringify(data, undefined, 2)}`);
                    this.canali.push(data);
                },
